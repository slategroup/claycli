 ...
 */
function lintURL(uri, isRecursive, prefix, concurrency, argv) {
  let spinner;

  spinner = logger.startSpinner('Linting url...');
  clay.checkAllReferences(uri, isRecursive, prefix, concurrency, argv)
...
function handler(argv) {
  if (argv.url) {
    return lintURL(argv.url, argv.recursive, config.getSite(argv.site), argv.concurrency, argv);
  } else if (argv.file) {
