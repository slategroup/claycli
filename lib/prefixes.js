...
function add(dispatch, prefix) {
  ...
  }).then((prefixedString) => replace(prefixedString, /"url":"(.*)"/g, (match, uri) => Promise.resolve(`"url":"${urlPrefix}${uri}"`)))).map(JSON.parse);
}
...
function remove(dispatch, prefix) {
  ...
  }).then((unprefixedString) => {
    if (prefix) {
      // sometimes prefix is null. why is that?
      return replace(unprefixedString, /"url":"(.*)"/g, (match, prefixedURI) => Promise.resolve(`"url":"${prefixedURI.replace(urlPrefix, '')}"`))
    }
