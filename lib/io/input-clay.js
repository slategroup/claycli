 ...
 */
function checkReference(concurrency, prefix, url, argv) {
  const key = config.getKey(argv.key);
  if (!_.isString(url)) {
  ...
  return rest.get(url, {concurrency, key})
    .map(() => ({ result: 'success' }))
 ...
 * @return {Stream} of { result: 'success or error', url (if error) }
 */
function recursivelyCheckReference(concurrency, prefix, uris, argv) {
  const key = config.getKey(argv.key);
  let urls;
  ...
  return rest.get(urls, {concurrency, key})
    .flatMap((data) => {
 ...
 * @return {Stream} of { result: 'success or error', url (if error) }
 */
function checkComponent(url, prefix, isRecursive, concurrency, argv) {
  const key = config.getKey(argv.key);
  const stream = rest.get(url, {concurrency, key})
    .flatMap((data) => {
  ...
  if (isRecursive) {
    return stream.flatMap(recursivelyCheckReference.bind(null, concurrency, prefix, argv));
  } else {
    return stream.flatMap(checkReference.bind(null, concurrency, prefix, argv));
  }
}
 ...
 * @return {Stream} of { result: 'success or error', url (if error) }
 */
function checkPage(url, prefix, isRecursive, concurrency, argv) {
  const key = config.getKey(argv.key);
  const stream = rest.get(url, {concurrency, key})
    .flatMap((data) => {
 ...
 * @return {Stream} of { result: 'success or error', url (if error) }
 */
function checkPublicUrl(url, prefix, isRecursive, concurrency, argv) {
  let publicUrl;
  ...
  return rest.get(publicUrl, {concurrency})
    .flatMap((pageUri) => {
      ...
      return checkPage(pageUrl, prefix, isRecursive, concurrency, argv);
    }).errors(pushRestError);
}
 ...
 * @return {Stream} of { result: 'success or error', url (if error) }
 */
function checkAllReferences(uri, isRecursive, prefix, concurrency, argv) {
  ...
  if (clayUtils.isComponent(url)) {
    return checkComponent(url, urlUtil.getUrlPrefix(url).prefix, isRecursive, concurrency, argv);
  } else if (clayUtils.isPage(url)) {
    return checkPage(url, urlUtil.getUrlPrefix(url).prefix, isRecursive, concurrency, argv);
  } else {
    return checkPublicUrl(url, prefix, isRecursive, concurrency, argv);
  }
}
 ...
 */
function getDeepComponent(concurrency, url, argv) {
  const key = config.getKey(argv.key);
  return rest.get(`${url}.json`, {concurrency, key})
    .map((data) => ({ url, data }));
}
/**
 ...
 * @return {Stream}
 */
function getDeepPage(concurrency, url, argv) {
  ...
  return rest.get(url, {concurrency, key})
    .flatMap((pageData) => {
      ...
      return h([
        ...
        h(childUrls).flatMap(getDeepComponent.bind(null, concurrency, argv))
      ]).merge();
    });
}
 ...
 */
function importUrl(url, concurrency, argv) {
  if (clayUtils.isComponent(url)) {
    return getDeepComponent(concurrency, url, argv);
  } else if (clayUtils.isPage(url)) {
    return getDeepPage(concurrency, url, argv);
  } else {
